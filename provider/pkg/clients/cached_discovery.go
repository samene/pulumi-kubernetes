...
func (d *CachedDiscoveryClient) ServerResourcesForGroupVersion(groupVersion string) (*metav1.APIResourceList, error) {
	...
	if err == nil {
		...
		if err := runtime.DecodeInto(Codecs.UniversalDecoder(), cachedBytes, cachedResources); err == nil {
			logger.V(10).Infof("returning cached discovery info from %v", filename)
	...
	if err != nil {
		logger.V(3).Infof("skipped caching discovery info due to %v", err)
		return liveResources, err
	}
	if liveResources == nil || len(liveResources.APIResources) == 0 {
		logger.V(3).Infof("skipped caching discovery info, no resources found")
		return liveResources, err
	}

	if err := d.writeCachedFile(filename, liveResources); err != nil {
		logger.V(1).Infof("failed to write cache to %v due to %v", filename, err)
	}
...
func (d *CachedDiscoveryClient) ServerGroups() (*metav1.APIGroupList, error) {
	...
	if err == nil {
		...
		if err := runtime.DecodeInto(Codecs.UniversalDecoder(), cachedBytes, cachedGroups); err == nil {
			logger.V(10).Infof("returning cached discovery info from %v", filename)
	...
	if liveGroups == nil || len(liveGroups.Groups) == 0 {
		logger.V(3).Infof("skipped caching discovery info, no groups found")
		return liveGroups, err
	}
...
func (d *CachedDiscoveryClient) writeCachedFile(filename string, obj runtime.Object) error {
	...
	bytes, err := runtime.Encode(Codecs.LegacyCodec(), obj)
	if err != nil {
		return err
	}
